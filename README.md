...
- **智能播放策略 (Auto Mode)**
  - **自适应优化**：内置智能调度算法，根据视频分辨率自动选择最佳解码模式。
    - **小分辨率 (<40万像素)**：自动切换至 **SSE 模式**，避免多线程开销。
    - **高清分辨率 (≥40万像素)**：自动切换至 **多线程模式**，释放并行计算潜能。
...
- **完善的播放控制**
  ...
  - **实时状态监控**：底部状态栏实时显示当前帧率 (FPS) 、每帧实时耗时及**平均耗时**。
...
### v1.1 (Latest)
- **新增**：智能策略 (Auto Mode)，根据分辨率自动选择最优解码方案。
- **新增**：平均耗时统计显示，提供更稳定的性能参考数据。
- **优化**：修复了 Y4M 格式在 seek 操作时的定位偏差问题。
- **优化**：改进了倍速播放的定时器逻辑，确保点击即播。
...
A: 这是正常的。创建和销毁线程本身有开销（Context Switch）。对于小图，计算量很小，线程开销占比大，反而拖慢速度；对于大图（1080p+），计算量占主导，多线程优势才能体现。**建议开启 Auto 模式，让程序自动帮您选择最优方案。**
